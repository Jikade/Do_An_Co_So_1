...
export default function NowPlayingPage() {
  ...
  const currentSong = nowPlaying || songs[0] || mockSongs[0];

  const lyrics =
    language === "vi" ? currentSong.lyricsVi || [] : currentSong.lyricsEn || [];

  const displayedDuration = totalDuration || currentSong.duration || 0;
    ...
    lyrics.length > 0
      ? Math.min(
          ...
          Math.max(
            0,
            Math.floor(
              (currentTime / Math.max(displayedDuration, 1)) * lyrics.length,
            ),
          ),
        )
      : 0;
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden rounded-[2rem]">
        ...
        <div
          ...
          style={{
            backgroundImage:
              'url(\'data:image/svg+xml;utf8,%3Csvg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg"%3E%3Cfilter id="noiseFilter"%3E%3CfeTurbulence type="fractalNoise" baseFrequency="0.9" numOctaves="3" stitchTiles="stitch"/%3E%3C/filter%3E%3Crect width="100%25" height="100%25" filter="url(%23noiseFilter)"/%3E%3C/svg%3E\')',
          }}
        ></div>
      </div>
      ...
      <div className="relative z-10 flex h-full flex-1 flex-col px-4 pb-8 pt-6 sm:px-6 md:px-8 xl:px-10">
        {songError && (
          <div className="mx-auto mb-4 w-full max-w-[1400px] rounded-2xl border border-yellow-500/20 bg-yellow-500/10 px-4 py-3 text-sm text-yellow-100">
            Không kết nối được backend, đang dùng dữ liệu mẫu.
          </div>
        )}

        {isLoadingSongs && (
          <div className="mx-auto mb-4 w-full max-w-[1400px] rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/70">
            Đang tải danh sách bài hát từ backend...
          </div>
        )}
        <div className="mx-auto flex w-full max-w-[1400px] flex-1 flex-col gap-6 xl:flex-row xl:items-stretch xl:justify-between">
          ...
          <section className="flex flex-1 flex-col rounded-[2.5rem] border border-white/[0.04] bg-white/[0.015] p-6 backdrop-blur-3xl shadow-[inset_0_1px_1px_rgba(255,255,255,0.05),0_30px_60px_rgba(0,0,0,0.5)] md:p-8 xl:max-w-[760px]">
            ...
            <div className="flex flex-1 flex-col items-center justify-center gap-10 md:gap-14">
              ...
              <div
                ref={coverRef}
                className="relative group w-full max-w-[280px] md:max-w-[340px]"
              >
                  ...
                  <div className="flex items-center justify-center gap-4 md:gap-7">
                    <button
                      ...
                      className={cn(
                        ...
                        isShuffled
                          ? "text-[var(--song-primary)] drop-shadow-[0_0_8px_var(--song-primary)]"
                    ...
                    >
                      ...
                      <div
                        className="absolute -inset-1 rounded-full opacity-0 blur-xl transition-opacity duration-500 group-hover:opacity-100"
                  ...
                  <div className="mt-8 flex items-center justify-between px-3">
                    <button
                      ...
                      className={cn(
                        ...
                        isFavorite
                          ? "text-pink-500 drop-shadow-[0_0_15px_rgba(236,72,153,0.8)]"
          ...
          <aside className="flex w-full flex-col gap-5 xl:w-[360px]">
            ...
            <div className="group flex items-center gap-4 rounded-[2rem] border border-white/[0.04] bg-white/[0.015] p-3 pl-4 backdrop-blur-xl shadow-lg transition-all duration-300 hover:bg-white/[0.03] hover:border-white/10 hover:shadow-2xl">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="pt-1">
                  <p className="text-[0.55rem] uppercase tracking-[0.25em] text-[var(--song-primary)] font-semibold">
                    {t ? t("nowPlaying") : "Hiện đang phát"}
                  </p>
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    {queueSongs.map((song, index) => (
                      <div
                        key={song.id}
                        ref={(el) => {
                          playlistItemsRef.current[index] = el;
                        }}
