...
import { 
  ...
  Play, Heart, MoreHorizontal, Search,
  Music2, Smile, ArrowUpRight, ListMusic,
  Disc3, Waves, Sparkles, LayoutGrid
} from 'lucide-react';
import { mockSongs, mockHistoryRecords, mockEmotions, type Language } from '@/lib/duLieuGiaLap';
import { useTheme } from '@/lib/nguCanhGiaoDien';
import { MoodBadge } from '@/components/huyHieuCamXuc';
import { cn } from '@/lib/tienIch';

// Helper to group by "human relative time"
const groupRecords = (records: typeof mockHistoryRecords, language: Language) => {
  ...
  records.forEach((record, i) => {
    ...
    else if (i < 8) groups[language === 'vi' ? 'Tuần này' : 'This Week'].push(record);
...
export default function HistoryPage() {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'moods' | 'sessions'>('all');
  ...
  const groupedData = useMemo(() => groupRecords(filteredHistory, language), [filteredHistory, language]);
    ...
    <div className="relative min-h-screen pb-40 text-white antialiased overflow-hidden">
        ...
        <div className="relative z-10 w-full p-8 md:p-14 grid gap-10 xl:grid-cols-[1fr_1.2fr] items-center">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 mb-6 backdrop-blur-md">
              ...
              <span className="text-[0.6rem] font-bold uppercase tracking-widest text-white/60">Listening Archive</span>
            </div>
            
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-white leading-tight">
               {language === 'vi' ? 'Ký Ức' : 'Your Music'}<br/>
               <span className="text-white/40">{language === 'vi' ? 'Gia Giai Điệu' : 'Session Journal'}.</span>
            </h1>
          ...
          <div className="grid grid-cols-2 gap-4">
             {[
               { label: language === 'vi' ? 'Số Bài Nghe' : 'Songs Played', val: '1,280', icon: Music2, color: 'text-[#4facfe]' },
               { label: language === 'vi' ? 'Tâm Trạng Chính' : 'Main Mood', val: 'Vui vẻ', icon: Smile, color: 'text-[var(--brand-accent)]' },
               { label: language === 'vi' ? 'Giờ Nghe' : 'Hours Sync', val: '46.2h', icon: Clock, color: 'text-[#a88beb]' },
               { label: language === 'vi' ? 'Chu Kỳ AI' : 'AI Accuracy', val: '92%', icon: Sparkles, color: 'text-[#ffb199]' },
             ].map((stat, i) => (
               <div key={i} className="group p-6 rounded-[2rem] border border-white/10 bg-white/[0.02] backdrop-blur-xl hover:bg-white/5 transition-all">
                  ...
                  <p className="text-[0.65rem] font-bold uppercase tracking-widest text-white/30 mt-1">{stat.label}</p>
               </div>
             ))}
          </div>
        </div>
      ...
      <section className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6 px-1">
            ...
            {['all', 'moods', 'sessions'].map((tab) => (
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab ? "bg-white text-black shadow-lg" : "text-white/40 hover:text-white hover:bg-white/5"
                )}
              >
                {tab === 'all' ? (language === 'vi' ? 'Tất cả' : 'All') : 
                 tab === 'moods' ? (language === 'vi' ? 'Tâm trạng' : 'Moods') : 
                 (language === 'vi' ? 'Phiên nghe' : 'Sessions')}
              </button>
            ))}
         </div>
         ...
         <div className="relative w-full md:w-[320px] group">
            ...
            <input 
               type="text" 
               placeholder={language === 'vi' ? "Tìm trong ký ức..." : "Search memory..."}
               value={searchQuery}
      ...
      >
         {groupedData.map(([groupTitle, items], groupIndex) => (
            <section key={groupTitle} className="space-y-6">
               ...
               <div className="flex items-center gap-6 px-2">
                  <h3 className="text-[0.7rem] font-black uppercase tracking-[0.4em] text-white/20 whitespace-nowrap">{groupTitle}</h3>
                  ...
                  {items.map((record, i) => {
                     ...
                     return (
                        <motion.div 
                          ...
                          onClick={() => { setNowPlaying(song); setIsPlaying(true); }}
                        >
                           {/* Time Pin */}
                           <div className="hidden md:flex flex-col items-center justify-center w-16 shrink-0 opacity-20 group-hover:opacity-100 transition-opacity">
                              <span className="text-[0.6rem] font-black tracking-widest">{record.timestamp.split(' ')[0]}</span>
                           ...
                           <div className="flex-1 min-w-0">
                              <h4 className="text-lg font-bold text-white group-hover:text-[var(--brand-accent)] transition-colors line-clamp-1">{song.title}</h4>
                              <p className="text-[0.85rem] font-medium text-white/40 mt-0.5">{song.artist}</p>
                           </div>

                           {/* Mood Badge (Refined) */}
                           <div className="hidden sm:flex shrink-0">
                              <MoodBadge mood={record.emotion as any} size="sm" className="bg-white/5 border-white/5 text-white/60" />
                           </div>
         ...
         {filteredHistory.length === 0 && (
            ...
            >
               ...
               <p className="text-white/30 font-medium">{language === 'vi' ? 'Không tìm thấy ký ức nào phù hợp.' : 'No memories found.'}</p>
            </motion.div>
         )}
      </motion.div>
      ...
      >
         ...
         <div>
            <h5 className="text-xl font-bold text-white mb-2">{language === 'vi' ? 'Hành trình trọn vẹn' : 'Complete Journey'}</h5>
            <p className="text-[0.9rem] text-white/40 max-w-md">{language === 'vi' ? 'Mọi nốt nhạc bạn từng nghe đều được AI lưu trữ và tối ưu cho những lần gợi ý tiếp theo.' : 'Every note you\'ve ever heard is archived and optimized for future recommendations.'}</p>
         </div>
