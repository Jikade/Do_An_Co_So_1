...
import {
  Activity, Play, Sparkles, Disc3, 
  Waves, ListMusic, AudioLines, Flame,
  Compass, ShieldCheck, Heart, Info, Clock, ArrowRight
} from 'lucide-react';
...
export default function RecommendationsPage() {
  ...
  const recommendations = useMemo(() => {
    ...
    if (activeTab === 'chill') items = items.filter(i => i.song.emotion === 'calm' || i.song.emotion === 'sad');
    if (activeTab === 'energy') items = items.filter(i => i.song.emotion === 'energetic' || i.song.emotion === 'happy');
    if (activeTab === 'deep') items = items.filter(i => i.song.emotion === 'nostalgic' || i.song.emotion === 'romantic');
    if (activeTab === 'discovery') items = items.filter(i => i.confidence < 90);
  ...
  const curatedPlaylists = useMemo(() => {
      // Pick playlists matching emotion for better curation feel
      ...
      >
        ...
        <motion.header variants={itemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-6">
           <div className="flex items-center gap-3">
              <div className="flex items-center gap-3 px-5 py-2.5 rounded-2xl bg-white/[0.03] border border-white/10 backdrop-blur-3xl shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
                 ...
                 <span className="text-[0.65rem] font-bold uppercase tracking-[0.2em] text-white/50">{language === 'vi' ? 'Trạng thái:' : 'Status:'}</span>
                 <span className="text-[0.7rem] font-black uppercase text-white tracking-widest">{currentEmotion}</span>
                 <span className="h-4 w-[1px] bg-white/10 mx-1" />
                 <ShieldCheck className="h-4 w-4 text-[var(--brand-accent)]" />
                 <span className="text-[0.65rem] font-bold uppercase tracking-[0.2em] text-[var(--brand-accent)]">Match 98.4%</span>
              </div>
           </div>
        ...
        {heroItem && (
          <motion.section 
            ...
            onClick={() => { setNowPlaying(heroItem.song); setIsPlaying(true); }}
          >
             {/* Huge Focal Image */}
             ...
             <div className="absolute inset-0 z-10 flex flex-col justify-end p-8 md:p-14">
                <div className="max-w-4xl space-y-6">
                   ...
                   >
                     ...
                     {language === 'vi' ? 'Tiêu Điểm Dành Riêng Cho Bạn' : 'Top Curated Selection'}
                   </motion.div>
                   ...
                   <div className="flex flex-col md:flex-row md:items-center gap-6 pt-4">
                      <div className="flex items-center gap-4">
                         <div className="h-12 w-12 rounded-full border border-white/20 overflow-hidden bg-white/5">
                            <img src={heroItem.song.coverUrl} className="w-full h-full object-cover opacity-60" alt="Artist" />
                         </div>
                         <p className="text-xl md:text-2xl font-semibold text-white/80">{heroItem.song.artist}</p>
                      </div>
        ...
        <motion.section variants={itemVariants} className="space-y-8">
           <div className="flex items-end justify-between px-2">
              <div className="space-y-2">
                 <p className="text-[0.65rem] font-bold uppercase tracking-[0.3em] text-[var(--brand-accent)]">{language === 'vi' ? 'Hòa Âm Không Gian' : 'Harmony Space'}</p>
                 <h3 className="text-3xl md:text-4xl font-black text-white">{language === 'vi' ? 'Playlist Hợp Mood Nhất' : 'Mood-Perfect Playlists'}</h3>
              </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_400px] gap-12">
            ...
            <motion.section variants={itemVariants} className="space-y-8">
               <div className="flex items-center gap-4">
                  <h3 className="text-2xl font-bold text-white">{language === 'vi' ? 'Giai Điệu Tương Đồng' : 'Similar Vibes'}</h3>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {priorityGrid.map((item, i) => (
                    <motion.div 
                      ...
                      onClick={() => { setNowPlaying(item.song); setIsPlaying(true); }}
                    >
                       {/* Rich Visual Mask */}
                       <div className="absolute inset-0">
                          <img src={item.song.coverUrl} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" alt="Cover" />
                       ...
                       <div className="absolute inset-0 p-6 flex flex-col justify-end">
                          <div className="space-y-1">
                             <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-[0.6rem] font-bold uppercase tracking-widest text-[#a88beb]">{item.song.emotion}</span>
                             </div>
                             <h4 className="text-xl font-bold text-white line-clamp-1">{item.song.title}</h4>
                             <p className="text-[0.8rem] text-white/50 font-medium">{item.song.artist}</p>
                          </div>
                       </div>
            ...
            <motion.section variants={itemVariants} className="space-y-8">
                <div className="flex items-center justify-between">
                   <h3 className="text-2xl font-bold text-white">{language === 'vi' ? 'Nghe Tiếp Theo' : 'Next Up'}</h3>
                ...
                <div className="flex flex-col gap-4 p-6 rounded-[2.5rem] bg-white/[0.015] border border-white/[0.04] backdrop-blur-3xl">
                   {vibeClusters.map((cluster, i) => (
                      <div 
                         key={cluster.id} 
                         onClick={() => { setNowPlaying(cluster.song); setIsPlaying(true); }}
                      ...
                      >
                         <div className="relative h-14 w-14 rounded-xl overflow-hidden shadow-lg border border-white/10 shrink-0">
                            <img src={cluster.song.coverUrl} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all" alt="Mini" />
                         </div>
                         <div className="min-w-0 flex-1">
                            <h5 className="text-[0.95rem] font-bold text-white truncate">{cluster.song.title}</h5>
                            <p className="text-[0.7rem] text-white/40 font-medium truncate">{cluster.song.artist}</p>
                         </div>
                         <div className="flex flex-col items-end opacity-20 group-hover:opacity-100 transition-opacity">
                            <span className="text-[0.6rem] font-black tabular-nums">{cluster.confidence}%</span>
                            <div className="h-1 w-10 bg-white/10 rounded-full overflow-hidden mt-1">
                               <div className="h-full bg-[var(--brand-accent)]" style={{ width: `${cluster.confidence}%` }} />
                            </div>
                         </div>
                      </div>
                   ))}
                ...
                <div className="p-6 rounded-[2rem] border border-[var(--brand-accent)]/20 bg-[var(--brand-accent)]/5">
                   <div className="flex items-start gap-4">
                      ...
                      <div>
                         <p className="text-[0.8rem] font-bold text-white">{language === 'vi' ? 'Bạn muốn dịu hơn?' : 'Want a softer vibe?'}</p>
                         <p className="text-[0.7rem] text-white/40 mt-1">{language === 'vi' ? 'Thử chọn filter "Chill Flow" để hệ thống làm mới dải tần số.' : 'Try the Chill Flow filter to refresh the frequency range.'}</p>
                      </div>
                   </div>
                </div>
            </motion.section>
