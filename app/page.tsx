...
import { 
  ArrowRight, Crown, Disc3, Play, Radio, 
  ScanFace, Waves, Sparkles, Heart, Music2,
  ChevronRight, Mic2, Activity, Zap,
  Orbit, Atom, CircleDot, Globe
} from 'lucide-react';
...
function CosmicCore() {
  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      const xSet = gsap.quickTo(containerRef.current, "x", { duration: 0.8, ease: "power3" });
      const ySet = gsap.quickTo(containerRef.current, "y", { duration: 0.8, ease: "power3" });
      ...
      const handleMouseMove = (e: MouseEvent) => {
        ...
        // Deep Parallax logic
        gsap.to(starfieldRef.current, { x: x * 0.5, y: y * 0.5, duration: 1.5 });
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 pointer-events-none overflow-hidden z-0 bg-[#020202]">
      ...
      <div ref={starfieldRef} className="absolute inset-[-5%] opacity-20">
         ...
         <div className="absolute inset-0 opacity-50" style={{ backgroundImage: 'radial-gradient(white 1.5px, transparent 1.5px)', backgroundSize: '120px 120px', backgroundPosition: '30px 30px' }} />
      </div>

      {/* LAYER 3: CENTRAL ENERGY CORE */}
      <div ref={coreRef} className="absolute inset-0 flex items-center justify-center">
        {/* Core Glow */}
      ...
      <div className="absolute inset-0 opacity-20">
         {[...Array(15)].map((_, i) => (
           <motion.div
             key={i}
             animate={{ y: [0, -200], x: [0, (Math.random() - 0.5) * 50], opacity: [0, 0.8, 0] }}
             transition={{ duration: 8 + Math.random() * 5, repeat: Infinity, delay: i * 0.4 }}
      ...
      <nav className="fixed top-0 inset-x-0 z-50 p-6 md:p-8 pointer-events-none">
        <div className="mx-auto max-w-[1600px] flex items-center justify-between pointer-events-auto">
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-3">
            ...
            <div>
               <p className="text-[0.5rem] font-black uppercase tracking-[0.3em] text-white/30 leading-none mb-1">{copy.brandEyebrow}</p>
               <h4 className="text-sm font-black tracking-tighter text-white">MoodSync AI</h4>
            </div>
          </motion.div>
          <div className="flex gap-4">
             <Link href="/app" className="px-6 py-2.5 rounded-full bg-white text-black text-[0.65rem] font-black uppercase tracking-widest hover:scale-105 transition-transform">Vào app</Link>
          </div>
        </div>
      </nav>
      ...
      <main className="relative z-10">
        ...
        >
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-md">
             ...
             <span className="text-[0.6rem] font-black uppercase tracking-[0.4em] text-white/60">{copy.heroBadge}</span>
          </div>
          ...
          <div ref={ctaRef} className="flex flex-col sm:flex-row gap-5 pt-4">
             <GlowCta href="/app" className="px-14 py-6 shadow-[0_20px_50px_rgba(255,255,255,0.15)]">
                <Play className="h-4 w-4 fill-current mr-2" />
                {copy.primaryCta}
             </GlowCta>
             <GlowCta href="/nhanDienCamXuc" variant="secondary" className="px-10 py-6">
        ...
        <div className="mx-auto max-w-[1400px] px-6 md:px-12 space-y-24 pb-40 -mt-10">
           ...
           <section className="space-y-10">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-4">
                 <div className="space-y-3">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-[0.65rem] font-black uppercase tracking-[0.3em] text-[var(--brand-accent)]">Vũ trụ Mood</span>
                    </div>
                    <h2 className="text-4xl md:text-5xl font-black tracking-tighter text-white">{copy.landingSections.emotions.title}</h2>
                 </div>
                 <p className="max-w-xs text-xs leading-relaxed text-white/40 italic">{copy.landingSections.emotions.description}</p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-5">
                 {moodCollections.map((mood) => (
                    ...
                    >
                       <div className={cn("absolute inset-0 bg-gradient-to-br opacity-10 group-hover:opacity-30 transition-opacity", mood.accent)} />
                       <p className="text-[0.7rem] font-black uppercase tracking-widest text-white/30 group-hover:text-white transition-colors">{localizedCopy(mood.title, 'vi')}</p>
                       <h4 className="text-lg font-black tracking-tight text-white/80">{localizedCopy(mood.title, 'vi')}</h4>
                    </motion.div>
                 ))}
                 ...
                 <div className="col-span-2 p-10 rounded-[3rem] bg-white text-black flex flex-col justify-center gap-4 relative overflow-hidden shadow-[0_30px_70px_rgba(255,255,255,0.1)]">
                    ...
                    <h3 className="text-2xl font-black tracking-tighter leading-tight italic">Cảm xúc là mã nguồn âm nhạc.</h3>
                    <p className="text-[0.75rem] font-medium leading-relaxed opacity-60">MoodSync AI học từ cảm xúc của bạn để kiến tạo danh sách phát cá nhân hóa tuyệt đối.</p>
                 </div>
              </div>
           </section>
              ...
              <div className="grid lg:grid-cols-2 gap-20 items-center relative z-10">
                 <div className="space-y-10">
                    <div className="inline-flex items-center gap-3 px-5 py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-md">
                       ...
                       <span className="text-[0.65rem] font-bold uppercase tracking-widest text-white/50">{copy.landingSections.personalization.eyebrow}</span>
                    </div>
                    <h2 className="text-5xl md:text-6xl font-black tracking-tighter leading-none">{copy.landingSections.personalization.title}</h2>
                    <p className="text-[1.05rem] text-white/40 leading-relaxed italic">{copy.landingSections.personalization.description}</p>
                    <div className="grid grid-cols-2 gap-8 pt-4">
                       <div>
                          <p className="text-4xl font-black tracking-tighter text-white">520+</p>
                          <p className="text-[0.6rem] font-bold uppercase tracking-widest text-white/20">Nghệ sĩ liên kết</p>
                       </div>
                       <div>
                          <p className="text-4xl font-black tracking-tighter text-white">Neural</p>
                          <p className="text-[0.6rem] font-bold uppercase tracking-widest text-white/20">Công nghệ lõi</p>
                       </div>
                    </div>
                 </div>
           ...
           <section className="text-center py-32 space-y-12">
              <div className="space-y-4">
                 <h2 className="text-5xl md:text-7xl font-black tracking-tighter italic">{copy.landingSections.final.title}</h2>
                 <p className="max-w-2xl mx-auto text-white/30 text-[1rem] font-medium leading-relaxed italic">{copy.landingSections.final.description}</p>
              </div>
              <div className="flex flex-col sm:flex-row justify-center gap-6">
                 <GlowCta href="/app" className="px-16 py-6 scale-110 shadow-[0_0_60px_rgba(30,215,96,0.2)]">Vào vũ trụ ngay</GlowCta>
                 <GlowCta href="/nhanDienCamXuc" variant="secondary" className="px-10 py-6 scale-110">Tìm hiểu thêm</GlowCta>
              </div>
           </section>
